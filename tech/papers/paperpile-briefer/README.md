...
- 任意でMarkdownをrepoへcommit/push（systemdでは無効が標準）
...
- 要約生成が失敗した場合、`state/seen.json`は更新されないため、次回に同じ論文が再処理されます。
- systemd実行では `PUSH_TO_GIT=false` を標準にし、GitHubへのcommit/pushは手動で行います。
